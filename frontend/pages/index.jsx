...
import { createReaction } from "../utils/api/create-reaction";
import { getBoard } from "../utils/api/get-board";
import { useAuth } from "../contexts/AuthContext";
import { useEffect, useState } from "react";

const Home = () => {
  const { accessToken, setError, profile } = useAuth();
  ...
  useEffect(() => {
    (async () => {
      if (!board) {
        const { profiles, error } = await getBoard(accessToken);
        ...
        if (profiles) {
          const applicableProfiles = profiles.filter((p) => p.uuid !== profile?.uuid);
          setBoard(applicableProfiles);
        }
      }
    })();
  }, [board]);

  const react = async (targetAccountId, type) => {
    const { reaction, error: reactionError } = await createReaction(
      { targetAccountId, type },
      accessToken,
    );
    ...
    <>
      <section className="grid place-items-center">
        {board?.length > 0 ? (
          <>
            ...
            <div className="btn-group mt-6 space-x-4">
              <div className="tooltip tooltip-bottom" data-tip="Like">
                <button
                  className="btn btn-circle btn-lg fill-green-500"
                  onClick={() => react(board[currentIndex].uuid, "positive")}
                >
              ...
              <div
                ...
                data-tip="Reject"
                onClick={() => react(board[currentIndex].uuid, "negative")}
              >
          ...
          <div className="card w-auto bg-base-100 text-center mt-20">
            <div className="card-body">
              <p className="font-extrabold text-xl">
                No new propositions, <span className="text-primary">try again later</span>
              </p>
            </div>
          </div>
        )}
